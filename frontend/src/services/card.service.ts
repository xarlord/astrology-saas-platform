...
export const cardService = {
  ...
  async generateCard(data: GenerateCardRequest): Promise<GeneratedCard> {
    const response = await api.post<{ success: true; data: GeneratedCard }>('/v1/cards/generate', data);
  ...
  async getPublicCard(shareToken: string): Promise<PublicCard> {
    const response = await api.get<{ success: true; data: PublicCard }>(`/v1/cards/public/${shareToken}`);
    return response.data.data;
  },
  ...
  async getCardOGMetadata(shareToken: string): Promise<CardOGMetadata> {
    const response = await api.get<{ success: true; data: CardOGMetadata }>(`/v1/cards/public/${shareToken}/og`);
    return response.data.data;
  },
  ...
  async getCardOGMetadataById(id: string): Promise<CardOGMetadata> {
    const response = await api.get<{ success: true; data: CardOGMetadata }>(`/v1/cards/shared/${id}/og`);
    return response.data.data;
  },
  ...
  async getCardHistory(limit = 20, offset = 0): Promise<CardHistoryResponse> {
    const response = await api.get<{ success: true; data: CardHistoryResponse }>('/v1/cards/history', {
      params: { limit, offset },
    });
    return response.data.data;
  },
   ...
   */
  async getRateLimitStatus(): Promise<{ count: number; limit: number; remaining: number; resetAt: string }> {
    const response = await api.get<{ success: true; data: { count: number; limit: number; remaining: number; resetAt: string } }>('/v1/cards/rate-limit');
    return response.data.data;
  },
};
