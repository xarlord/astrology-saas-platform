...
const SynastryPage: React.FC<SynastryPageProps> = ({ charts }) => {
  ...
  const handleToggleFavorite = useCallback(async (id: string, isFavorite: boolean) => {
    try {
      ...
      setReports((prev) => prev.map((r) => (r.id === id ? { ...r, isFavorite: !isFavorite } : r)));
    } catch (err) {
  ...
  const getChartName = useCallback(
    (chartId: string) => {
    ...
    <div className="synastry-page" data-testid="synastry-page">
      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-gray-500 dark:text-gray-400">
          Compare two charts to discover relationship dynamics and compatibility
        </p>
      </div>
      ...
      {view === 'history' && (
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="p-4 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg text-center">
              {error}
            </div>
          ) : reports.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-6">
                Compare two charts to save your first compatibility report
              </p>
              <button className="primary-button" onClick={() => handleViewChange('calculator')}>
                Go to Calculator
              </button>
            </div>
          ) : (
              ...
              <div className="reports-list" data-testid="synastry-reports-list">
                {reports.map((report) => (
                  <div
                    ...
                    data-testid={`synastry-report-${report.id}`}
                  >
                    <div className="report-header">
                      ...
                      <div
                        className={`score-badge ${getScoreColor(report.overallCompatibility)}`}
                        data-testid={`report-score-${report.id}`}
                      >
                    ...
                    <div className="flex items-center gap-2 px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                      <button
                        ...
                        onClick={() => {
                          void handleToggleFavorite(report.id, report.isFavorite ?? false);
                        }}
