...
export const TransitReportCard: React.FC<TransitReportCardProps> = ({ className = '' }) => {
  ...
  if (!isPremiumUser) {
    return (
      <motion.div
        className={`bg-cosmic-card-solid/70 backdrop-blur-md border border-white/10 rounded-2xl p-6 ${className}`}
        ...
        <div className="bg-cosmic-page rounded-lg p-3 mb-4 border border-white/5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {['Key Dates', 'Love & Career', 'Health', 'Growth'].map((tag) => (
              <span
                key={tag}
                className="bg-cosmic-active px-2 py-0.5 rounded text-[10px] text-slate-400 border border-white/5"
              >
                {tag}
              </span>
            ))}
