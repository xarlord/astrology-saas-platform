          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div className="flex gap-0.5">
              {likelihoodBars.map((_, i) => (
                <span key={`likelihood-bar-${prediction.category}-${i}`} className={`text-[0.6rem] ${i < prediction.likelihood ? 'text-primary' : 'text-slate-200'}`}>&#9632;</span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              {forecast.journalPrompts.map((prompt, i) => (
                <div key={`journal-prompt-${forecast.theme}-${i}`} className="flex gap-4 p-5 bg-white/15 rounded-xl">
