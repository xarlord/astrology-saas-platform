  ...
  return (
    <div className="space-y-6">
      ...
      <section className="glass-panel rounded-2xl p-6">
        ...
        <p className="text-slate-200 leading-relaxed m-0 mb-4">{interpretation.sunHouse.interpretation}</p>
      ...
      <section className="glass-panel rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold text-white m-0">Moon Phase: {interpretation.moonPhase.phase.replace('-', ' ')}</h3>
        </div>

        <p className="text-slate-200 leading-relaxed m-0 mb-4">{interpretation.moonPhase.interpretation}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {interpretation.luckyDays.slice(0, 10).map((day, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-white/15 rounded-lg">
                ...
                <div className="text-sm text-slate-200 flex-1 mx-3 truncate">{day.reason}</div>
                <div className="flex items-center gap-1.5 text-sm">
                  <span
                    className="w-2.5 h-2.5 rounded-full shrink-0"
                    style={{ backgroundColor: getIntensityColor(day.intensity) }}
                  />
                  <span className="text-slate-200">{getIntensityLabel(day.intensity)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="text-center pt-4">
        <p className="text-sm text-slate-200 italic max-w-2xl mx-auto">
