      ...
      for (const [aspectType, orb] of Object.entries(aspectOrbs)) {
        ...
        if (diff <= orb) {
          aspects.push({
            ...
            weight: calculateAspectWeight(
              p1 as Planet,
              p2 as Planet,
              aspectType as SynastryAspect['aspect'],
            ),
            soulmateIndicator: isSoulmateAspect(
              p1 as Planet,
              p2 as Planet,
              aspectType as SynastryAspect['aspect'],
            ),
...
export function calculateCategoryScores(chart1: Chart, chart2: Chart): CompatibilityScores {
  ...
  const romanticAspects = aspects.filter(
    (a) =>
  ...
  const communicationAspects = aspects.filter(
  ...
  const emotionalAspects = aspects.filter(
    (a) =>
  ...
  const intellectualAspects = aspects.filter(
    (a) =>
  ...
  const spiritualAspects = aspects.filter(
    (a) =>
  ...
  const valuesAspects = aspects.filter(
    (a) => ['venus', 'saturn'].includes(a.planet1) || ['venus', 'saturn'].includes(a.planet2),
  );
 ...
 */
export function generateCompatibilityReport(chart1: Chart, chart2: Chart): CompatibilityReport {
...
function isSoulmateAspect(planet1: Planet, planet2: Planet, aspect: string): boolean {
  ...
  const key = `${planet1}-${planet2}-${aspect}`;
  return (
    soulmateAspects.includes(key) || soulmateAspects.includes(`${planet2}-${planet1}-${aspect}`)
  );
}
...
function getRelationshipDynamics(
...
function getCompositeChartInterpretation(_compositePlanets: {
  [key in Planet]?: CompositePlanet;
}): string {
  return (
    'The composite chart represents the relationship itself as a separate entity. ' +
...
function generateDetailedReport(
  ...
  report +=
    `Fire: ${elementalBalance.fire}, Earth: ${elementalBalance.earth}, ` +
